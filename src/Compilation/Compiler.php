...
readonly class Compiler
{
    ...
    public function compile(array $config): array
    {
        ...
        foreach ($entitiesConfig as $entityClass => $entityConfig) {
            ...
            foreach ($actionsConfig as $actionValue => $actionConfig) {
                ...
                $compiled[$entityClass][$action->value] = $this->getCompiler($entityAction)
                    ->compile(
                        $schema,
                        $entityAction,
                        $actionConfig,
    ...
    private function getCompiler(
        ...
        return $this->actionDefinitionCompilers[$entityAction->getAction()->value]
            ??
            throw new CrudEngineUnsupportedActionException($entityAction);
    }
}
